...
class ModelTrainer(object):
    ...
    def _model_hash(self, matrix_metadata, class_path, parameters):
        ...
        logging.debug("Creating model hash from unique data %s", unique)
    ...
    def _write_model_to_db(
        ...
        if model_id and not self.replace:
            ...
            if model_id:
                ...
                logging.debug("Added new model id %s", model_id)
            session.close()

        logging.debug("Saving feature importances for model_id %s", model_id)
        self._save_feature_importances(
            model_id, get_feature_importances(trained_model), feature_names
        )
        logging.debug("Done saving feature importances for model_id %s", model_id)
        return model_id
    ...
    def _train_and_store_model(
        ...
        logging.debug("Training and storing model for matrix uuid %s", matrix_store.uuid)
        ...
        logging.debug("Cached model: %s", model_hash)
        ...
        logging.debug("Wrote model to db: hash %s, got id %s", model_hash, model_id)
        return model_id
    ...
    def process_train_task(
        ...
        if (
            ...
            logging.debug("Skipping %s/%s", class_path, parameters)
    ...
    def generate_train_tasks(self, grid_config, misc_db_parameters, matrix_store=None):
        ...
        logging.debug("Found %s unique model training tasks", len(tasks))
        return tasks
